...
def build_label_manifold(train_data, train_p_target, k):
    ...
    M_extend_reg = M #M + 1e-6 * np.eye(M.shape[0])  # 强制正则化
    ...
    lb = np.full(num, -100000000)  # 不能设置inf，太大模型会爆，所以缩小8个0
    ub = np.full(num, 1000000000)
    ...
    obj = cp.Minimize(0.5 * cp.quad_form(x, H))
    # 同时满足以下约束条件：
